function initApplication() {
    var idb = require("./bdd/idb");
        ...
        animation.animateClick('.animateClick');
            ...
            GetPromotionFromSociete().then(function (oMarque) {
                ...
                GetAllFromDossier(oMarque.dossier_id).then(function (aDoss) {
                    if (aDoss.length > 0) {
                        ...
                        WinJS.Navigation.navigate('../../pages/document/document.html', {
                            docs: aDoss,
                            breadcrumb: [{item: "Promotions"}]
                        });
                    }
                });
            });
    ...
    function CreateIdbBibliodoc() {
        ...
        idb.CreateDatabase(Elipce.Bdd.nom, aStruct);
            ...
            // BDD a au moins une table
                ...
                try {
                    ...
                    transaction.oncomplete = function (e) {
                        if (aData.length > 0) {
                            WinJS.Navigation.navigate('./marque.html', {
                                marques: aData,
                                breadcrumb: [{item: 'MARQUES'}, {item: lettre.toUpperCase()}]
                            });
                        }
                        else {
                            // Message
                            alertMssg("Aucune marque déclarée pour " + lettre, null, "Recherche", "Fermer");
                        }
                    }
                }
                catch (ex) {
    ...
    function GetAllFromDossier(idDos) {
        ...
        return new WinJS.Promise(function (ok, ko) {
            ...
            idb.Read(Elipce.Bdd.nom, ['dossier'], 'dossier', 'dossier_id', range).then(function (aDoss) {
                ...
                // Lecture Documents
                return idb.Read(Elipce.Bdd.nom, ['document'], 'document', 'dossier_id', range);
...
module.exports.CreateIdbBibliodoc = CreateIdbBibliodoc;
module.exports.RechercherMarques = RechercherMarques;
module.exports.GetAllFromDossier = GetAllFromDossier;
